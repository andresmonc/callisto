...
public class HeadersTabView extends StackPane {
    ...
    private void addPropertyListeners(ObservableList<HeaderRow> headerObservableList) {
        headerObservableList.addListener((javafx.collections.ListChangeListener<HeaderRow>) change -> requestController.updateAllHeaders(headerObservableList));
        for (HeaderRow header : headerObservableList) {
            header.key.addListener((observable, oldValue, newValue) -> requestController.updateAllHeaders(headerObservableList));
            header.value.addListener((observable, oldValue, newValue) -> requestController.updateAllHeaders(headerObservableList));
            header.description.addListener((observable, oldValue, newValue) -> requestController.updateAllHeaders(headerObservableList));
        }
    }
    ...
    public void initialize(ApiRequest request) {
        tableView.setItems(FXCollections.observableArrayList(new HeaderRow("Authorization", "Bearer token", "Yomamaa token")));
        tableView.refresh();
