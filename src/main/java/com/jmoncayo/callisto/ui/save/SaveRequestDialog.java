...
public class SaveRequestDialog {
    ...
    public void open(Stage owner) {
        ...
        Platform.runLater(() -> {
            ...
            VBox layout = new VBox(10, new Label("Select a collection or create a new one:"), collectionList, newCollectionField, saveButton, cancelButton);
            layout.setPadding(new Insets(10));
            stage.setScene(new Scene(layout, 400, 300));

            // Load collections and show the dialog
            List<String> collections = collectionService.getCollections().stream().map(Collection::getName).toList();
